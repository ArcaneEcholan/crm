...
<script type="text/javascript">
	...
	$(function(){
		...
		//线索关联市场活动按钮（实质是插入数据到关系表中）
		$("#relateActivities").click(function() {

			//拼接参数：clueId & actId & actId & ...
			var $xz = $(":input[name=xz]:checked"); 	//获取挑了钩的复选框
			var param = "";
			...
			//发送请求
			$.get("clueServlet", "action=relateClueAndActivities&" + param,
			function (data) {
		...
		$("#aname").keydown(function(event) {
			if(event.keyCode == 13) {
				//过后台查出所有市场活动
				$.get("clueServlet", {
					"action":"getAllActivitiesByNameAndNotRelatedWithClue",
					...
					$.each(data, function (i,n) {
						...
						html += '<tr>';
						//复选框需要有活动id，一遍关联该活动；需要有名字，以便获取该复选框
						html += '<td><input type="checkbox" name="xz" value="' + n.id + '"/></td>';
						html += '<td>'+n.name+'</td>';
						html += '<td>'+n.startDate+'</td>';
						html += '<td>'+n.endDate+'</td>';
						//n.owner为所有者的名字，需要多表联查获取
						html += '<td>'+n.owner+'</td>';
						html += '</tr>';
					});
...
<body>
		...
		<div class="modal-dialog" role="document" style="width: 80%;">
			<div class="modal-content">
				...
				<div class="modal-footer">
					...
					<button type="button" class="btn btn-primary" id="relateActivities">关联</button>
				</div>
			</div>
		</div>
	</div>
	...
	<div style="position: relative; left: 40px; top: -30px;">
		...
		<div style="position: relative; height: 50px; width: 500px;  top: -72px; left: 700px;">
			<button type="button" class="btn btn-default" onclick="window.location.href='workbench/clue/convert.jsp?id=${clue.id}&fullname=${requestScope.clue.fullname}&appellation=${requestScope.clue.appellation}&company=${requestScope.clue.company}&owner=${requestScope.clue.owner}';"><span class="glyphicon glyphicon-retweet"></span> 转换</button>
...
<%--		如果没有被修改过，就不展示--%>
		<c:if test="${requestScope.clue.editTime != null}">
